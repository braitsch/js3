...
function init()
{
	...
	stage.background = '#D5DED9';
	addBalls();	onStart();
}

function addBalls()
{
	var n = stage.width / joints;
	for (var i=0; i < joints; i++) {
		...
		var c2 = new JS3Circle();
			...
			c2.y = Math.random() * stage.height;
			c2.dragStart = onDragStart;
			c2.dragComplete = onDragComplete;				
		sm.push(c2);
		ln.push(new JS3Line({strokeColor:line[st], strokeWidth:2}));
	};
	for (var i=0; i < joints; i++) stage.addChild(ln[i]);
	for (var i=0; i < joints; i++) stage.addChild(sm[i]);
	for (var i=0; i < joints; i++) stage.addChild(lg[i]);
}
